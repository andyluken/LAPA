...
def compute_egomotion_heatmap(
    ...
    peak = float(mag.max())
    if peak < STATIONARY_THRESHOLD:
        return np.zeros((3, patch_h, patch_w), dtype=np.float32)

    def to_patch(arr: np.ndarray) -> np.ndarray:
        return cv2.resize(arr.astype(np.float32) / peak, (patch_w, patch_h),
                          interpolation=cv2.INTER_AREA).astype(np.float32)

    return np.stack([to_patch(mag), to_patch(fx_raw), to_patch(fy_raw)], axis=0)
