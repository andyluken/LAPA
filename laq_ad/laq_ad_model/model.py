...
class CANBusHead(nn.Module):
    """Per-axis CAN bus prediction: each FSQ axis independently predicts one signal.
    ...
    Linear(fsq_dim, 2) head where swapping which axis predicts yaw vs. speed
    ...
    def __init__(self, fsq_dim: int, hidden_dim: int = 64):
        super().__init__()
        # Bind only the first N_CAN_SIGNALS axes to CAN predictions.
        # Extra FSQ axes (fsq_dim > 2) are left free — shaped by the flow loss.
        # hidden_dim is ignored (kept for API compatibility).
        n = min(fsq_dim, self.N_CAN_SIGNALS)
    ...
    def forward(self, z_q: torch.Tensor) -> torch.Tensor:
        """z_q: (b, fsq_dim) → (b, N_CAN_SIGNALS) predicted [yaw_rate_norm, speed_norm].

        Only the first N_CAN_SIGNALS FSQ axes are used; remaining axes are free.
        """
...
class LAQADModel(nn.Module):
    ...
    Architecture:
        ...
        → CNN compress (8×8 → 1) → project_in → FSQ → project_out
    ...
    def _quantize(self, last_tokens: torch.Tensor):
        ...
        Returns:
            z_q:      (b, fsq_dim) quantized values (straight-through grad)
            indices:  (b,) flat codebook indices
            z_bounded:(b, fsq_dim) continuous bounded values (for entropy reg)
            z_pre:    (b, fsq_dim) pre-tanh projection (for spread reg)
        """
        ...
        z_pre = torch.cat([h(x) for h in self.project_in], dim=-1)  # (b, fsq_dim)
