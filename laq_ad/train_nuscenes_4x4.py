"""LAQ-AD trainval experiment: FSQ levels=[4,4] → 16 codes.

Same perAxisCAN config as the best [3,3] run (NMI=0.4007) but with 4 levels
per axis instead of 3, giving a 4×4=16-code grid.  The extra resolution should
...
WARMUP_STEPS      = 500
RESULTS_FOLDER    = "results_laq_ad_trainval_4x4_perAxisCAN"
RESUME_FROM       = None       # fresh run

# 4×4 grid → 16 codes; per-axis heads force axis0=yaw, axis1=speed
LEVELS = [4, 4]
...
print(f"  LAQ-AD codebook size: {model.codebook_size}")  # 16
